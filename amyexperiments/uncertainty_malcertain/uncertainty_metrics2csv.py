...
if __name__ == '__main__':
    llm_name = "Salesforce/codegen-6B-multi"
    for dataset_name in ["vulcure_vd_random", "vulcure_vd_dataset"]:
        for data_type in ["test", "correct"]:
            if not os.path.isfile(
                    os.path.join("feature_csv", f"{dataset_name}/{data_type}/{llm_name}.csv")):
                uc_metrics, uc_metrics_name = main_(llm_name=llm_name, dataset_name=dataset_name, data_type=data_type)
