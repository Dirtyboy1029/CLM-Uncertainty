...
class ClassificationDataset:
    """
    An abstract base Source_Data for sequence classification problems. Multiple
    ...
    def __init__(
        ...
        ).input_ids[:, -1:]  # assume these encode to single tokens
        print('============================')
...
class Book_MIA(ClassificationDataset):
    def __init__(
        ...
        print('load file from ', os.path.join('/home/nusbac/LHD_LLM/uncertainty/LLM_uncertainty/bayesian_peft/database',
                                              file_path + '.jsonl'))
        dset = datasets.Dataset.from_json(
            os.path.join('/home/nusbac/LHD_LLM/uncertainty/LLM_uncertainty/bayesian_peft/database',
                         file_path + '.jsonl'))
...
class Prog_Lang(ClassificationDataset):
    def __init__(
            self,
            tokenizer: AutoTokenizer,
            add_space: bool = True,
            max_seq_len: int = 512,
            file_path: str = "proglang"
    ):
        print('load file from ',
              os.path.join('./database', file_path + '.jsonl'))
        dset = datasets.Dataset.from_json(
            os.path.join('./database', file_path + '.jsonl'))

        super().__init__(dset, tokenizer, 2, self.preamble, add_space, max_seq_len=max_seq_len)

    preamble = """You are an expert programmer. Determine whether the following code is written in Python (0) or Java (1).
...
Answer: """
    ...
    def clm_collate_fn(self, batch):
        # No need to use self.add_space here since we add it to the target tokens
        prompts = [self.preamble.format(sentence=e["code"]) for e in batch]
        prompts = self._tokenize_prompts(prompts)
...
class Code_Clone_Dataset(ClassificationDataset):
    def __init__(
            self,
            tokenizer: AutoTokenizer,
            add_space: bool = True,
    ...
    preamble = "Analyze the following two code snippets and determine whether they are code clones. " \
               "Output 1 if the two code snippets are semantically equivalent or represent the same logic (code clone), otherwise output 0.\n" \
               "Code Snippet A: {sentence_a}\n" \
               "Code Snippet B: {sentence_b}\n" \
               "Output: "
    ...
    def s2s_collate_fn(self, batch):
        prompts = [e["func"] for e in batch]
        prompts = self._tokenize_prompts(prompts)
        targets = t.tensor([e["multi_label"] for e in batch])
        return prompts, targets, targets


code_clone_dataset = Code_Clone_Dataset


class Multi_CWE_Primevul_Dataset(ClassificationDataset):
    def __init__(
            self,
            tokenizer: AutoTokenizer,
            add_space: bool = True,
        ...
        print('load file from ',
              os.path.join('/opt/data/private/LHD_LLM/LLM_uncertainty/bayesian_peft/database', file_path + '.jsonl'))
        dset = datasets.Dataset.from_json(
            os.path.join('/opt/data/private/LHD_LLM/LLM_uncertainty/bayesian_peft/database', file_path + '.jsonl'))

        super().__init__(dset, tokenizer, 4, self.preamble, add_space, max_seq_len=max_seq_len)

    preamble = "Analyze the provided code snippet and classify its CWE weakness type. " \
               "Output only a single number: 0 for CWE-119 (Buffer Overflow), 1 for CWE-20 (Improper Input Validation), 2 for CWE-416 (Use After Free), or 3 for CWE-189 (Numeric Handling Issues).\n" \
               "Code: {sentence}\n" \
               "Output: "

    def clm_collate_fn(self, batch):
        prompts = [self.preamble.format(sentence=e["func"]) for e in batch]
        prompts = self._tokenize_prompts(prompts)
        classes = t.tensor([e["multi_label"] for e in batch])
        targets = t.cat([self.label2target[e["multi_label"]] for e in batch])
        return prompts, classes, targets
...
my_multi_cwe_primevul_dataset = Multi_CWE_Primevul_Dataset


class Binary_CWE_Dataset(ClassificationDataset):
    def __init__(
            self,
            tokenizer: AutoTokenizer,
            add_space: bool = True,
    ...
    preamble = "Analyze the code snippet and classify its CWE weakness type. " \
               "Output only a single number: 0 for CWE-89 (SQL Injection), 1 for CWE-79 (Cross-Site Scripting)\n" \
               "Code: {sentence}\n"
...
binary_cwe_dataset = Binary_CWE_Dataset


class Primevul_Dataset(ClassificationDataset):
    def __init__(
            self,
            tokenizer: AutoTokenizer,
            add_space: bool = True,
    ...
    preamble = "Analyze the code for security issues. Output 1 if vulnerable, else 0.\n" \
               "Code: {sentence}\n" \
               "Output: "
...
primevul_dataset = Primevul_Dataset


class Code_Smell_Dataset(ClassificationDataset):
    def __init__(
            self,
            tokenizer: AutoTokenizer,
            add_space: bool = True,
    ...
    preamble = "Analyze the code for code smells. Output 0 if it is ComplexMethod, else 1.\n" \
               "Code: {sentence}\n" \
               "Output: "
...
codesmell_dataset = Code_Smell_Dataset


class VulCertain_Dataset(ClassificationDataset):
    def __init__(
            self,
            tokenizer: AutoTokenizer,
            add_space: bool = False,
        ...
        print('load file from ',
              os.path.join('/home/lhd/bayesian-peft/database', file_path + '.jsonl'))
        dset = datasets.Dataset.from_json(
            os.path.join('/home/lhd/bayesian-peft/database', file_path + '.jsonl'))

        super().__init__(dset, tokenizer, 2, self.preamble, add_space, numerical=False, boolean=True,
                         max_seq_len=max_seq_len)

    # For each sentence below, indicate whether it has been previously encountered (1) or is new (0).

    preamble = "Analyze the following code and determine whether it is vulnerable.\n" \
               "Output yes if vulnerable, otherwise no.\n" \
               "Code: {sentence}\n" \
               "Output: "

    # preamble = "Code: {sentence}"

    def clm_collate_fn(self, batch):
        # No need to use self.add_space here since we add it to the target tokens
        prompts = [self.preamble.format(sentence=e["func"]) for e in batch]
        prompts = self._tokenize_prompts(prompts)
        classes = t.tensor([1 if e["target"] == 'yes' else 0 for e in batch])
        targets = t.cat([self.label2target[1 if e["target"] == 'yes' else 0] for e in batch])
        return prompts, classes, targets
    ...
    #     cls_labels = [1 if item == 'yes' else 0 for item in targets]
    ...
    #     labels = []
    #
    #     for p_ids, a_ids in zip(prompt_enc["input_ids"], answer_enc["input_ids"]):
    #         ids = p_ids + a_ids
    #         mask = [1] * len(ids)
    #         lbls = [-100] * (len(p_ids) - 1) + a_ids + [-100]
    ...
    #         if pad_len > 0:
    #             input_ids[i] = [self.tokenizer.pad_token_id] * pad_len + input_ids[i]
    #             attention_mask[i] = [0] * pad_len + attention_mask[i]
    #             labels[i] = [-100] * pad_len + labels[i]
    #     return {
    #         "input_ids": t.tensor(input_ids),
    #         "attention_mask": t.tensor(attention_mask),
    #         "labels": t.tensor(labels),
    #         "cls_labels": t.tensor(cls_labels)
    #     }
    #
    #     # return prompts, targets, targets


vulcertain_dataset = VulCertain_Dataset
...
class LMDataset:
    """
    An abstract base Source_Data for autoregressive language modelling problems,
