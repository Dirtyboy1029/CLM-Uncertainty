...
class TFBLoRA(WrapperBase):
        ...
        if not args.ood_ori_dataset:
            self.load_adapter(
                os.path.join("checkpoints",
                             f"{args.model_type}_blob/{args.model}/{args.dataset.replace('_tfblora', '')}/{args.checkpoint_dic_name}"),
                adapter_name='default')
            print(f'LoRA Model loaded successfully from: ',
                  os.path.join("checkpoints",
                               f"{args.model_type}_blob/{args.model}/{args.dataset.replace('_tfblora', '')}/{args.checkpoint_dic_name}"))
        else:
            self.load_adapter(args.load_lora_path, adapter_name='default')
            print(f'LoRA Model loaded successfully from: ', args.load_lora_path)
        print('=====================')
    ...
    def fit(self, anchor_loader, target_ratio=0.01, max_iters=10):
        ...
        with torch.no_grad() and torch.inference_mode():
            ...
            for i_iter in range(max_iters):
                ...
                loss_change_ratio = (abs(
                    current_nll_loss.item() - ori_nll_loss.item()) / ori_nll_loss.item()) / self.all_ori_predicted_classes.size(
                    0)
    ...
    def evaluate(self, eval_loader):
        ...
        for step, batch in enumerate(eval_loader):
            with torch.no_grad() and torch.inference_mode():
                ...
                brier = (probs_stochastic - F.one_hot(labels, num_classes=logits_stochastic.size(-1))).pow(2).sum(
                    dim=-1).mean()
                briers.update(brier)
    ...
    def fit_evaluate(self):
        ...
        if self.args.iter == 0:
            ...
            all_ori_probs = []
            print('Init beta: ', self.args.bayes_beta)
            from tqdm import tqdm

            with torch.no_grad(), torch.inference_mode():
                for i, batch in enumerate(
                        tqdm(self.train_loader, desc="Inference", total=len(self.train_loader))
                ):
            ...
            self.fit(self.train_loader, target_ratio=self.args.th, max_iters=self.args.iter)
